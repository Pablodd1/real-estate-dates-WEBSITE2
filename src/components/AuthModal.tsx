...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[400px] bg-transparent-elevated border-gold/30 text-white p-0 overflow-hidden rounded-2xl">
        <div className="bg-gradient-to-b from-gold/10 to-transparent p-8 pt-10">
          <DialogHeader className="mb-6">
            ...
            <DialogDescription className="text-white/80 text-center mt-2 uppercase tracking-[0.2em] text-[10px] font-bold">
              {isReset 
            ...
            {!isLogin && !isReset && (
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  required
                  className="w-full liquid-glass  rounded-xl px-4 py-3 text-sm text-white focus:border-gold/50 focus:ring-1 focus:ring-gold/20 outline-none transition-all placeholder:text-white/70"
          ...
          <div className="mt-8 flex flex-col gap-4">
            {!isReset && (
              <button 
                ...
                type="button"
                disabled={loading}
                className="w-full bg-white/5  text-white font-bold py-3.5 rounded-xl hover:bg-white/10 transition-all flex items-center justify-center gap-3 disabled:opacity-50 text-xs"
              >
            ...
            <div className="flex flex-col items-center gap-3 mt-2">
              {!isReset && isLogin && (
                <button 
                  ...
                  className="text-white/75 hover:text-gold transition-colors text-[10px] uppercase tracking-widest font-bold"
                >
                  Lost your key?
                </button>
              )}
