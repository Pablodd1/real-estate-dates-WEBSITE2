...
export default function FeatureCard({ icon, iconColor = 'text-gold', title, description, tags, variant = 'default' }: FeatureCardProps) {
  if (variant === 'glass') {
    return (
      <div className="bg-white/5  rounded-xl p-8 hover:-translate-y-1 transition-all duration-300">
  ...
  return (
    <div className="liquid-glass  rounded-xl p-8 hover:-translate-y-1 hover:shadow-glow transition-all duration-300">
      <div className={`w-12 h-12 ${iconColor} mb-5`}>{icon}</div>
      <h4 className="text-xl font-medium text-white mb-3">{title}</h4>
